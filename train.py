...
def train(fold_index=1, num_classes=5, model_name='resnet101', checkPoint_start=0, lr=3e-4, batch_size=36):
    #Build the model:
    ...
    log.write(
            ' rate     iter   epoch |   valid   train   batch  |  time          \n')
    log.write(
            '-----------------------------------------------------------------\n')
    ...
    while i < MAX_BATCH:
        for batchdata in train_loader:
            ...
            if i % iter_valid==0:
                ...
                log.write(
                    '%0.5f %5.2f k %5.2f  | %0.3f    %0.3f    %0.3f | %s \n' % ( \
